...
$(function() {
		...
		try{
			...
			timeoutId = setTimeout(function() {
				try{
						...
						$.ajax({
							...
							success : function(data, status, jqXHR) {
								try{
									...
									smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_auto_complete ' + url + ']', null, error);
								}			
							},
					...
					smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_auto_complete setTimeout]', null, error);
				}			
			}, 500);
		}catch(error){
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_auto_complete]', null, error);
		}			
	});
		...
		try{
			...
			setTimeout(function() {
				try{
					...
					smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_auto_complete focusout setTimeout]', null, error);
				}			
			}, 500);
		}catch(error){
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_auto_complete focusout]', null, error);
		}			
	});
	...
	$('input.js_auto_complete').live('keydown', function(e) {
		try{
			...
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_auto_complete keydown]', null, error);
		}			
	});
	...
	$('.nav_srch_list').find('li').live('hover', function(e){
		try{
			...
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search nav_srch_list hover]', null, error);
		}			
	});
	...
	$('#upload_work_list').find('li').live('hover', function(e){
		try{
			...
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search upload_work_list hover]', null, error);
		}			
	});
	...
	$('div.js_chatter_list').find('li').live('hover', function(e){
		try{
			...
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_chatter_list hover]', null, error);
		}			
	});
	...
	$('div.js_srch_x').live('click', function(e) {
		try{
			...
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_srch_x]', null, error);
		}			
		return false;
	});
	...
	$('.js_select_community').live( 'click', function(e) {
		try{
			...
			if(isEmpty(communityItems) && !isEmpty(selectedApproverInfo)){
				...
				var isSameId = false;
				if (isEmpty(communityItems) || (!isEmpty(userField) && userField.attr('multiUsers') !== 'true') || (!isEmpty(departmentField) && departmentField.attr('multiUsers') !== 'true')){
				...
				if(!isSameId){
					...
					if(!isEmpty(searchFilter)){
						...
						departmentField.find('input[name="txtFilterStringOperand"]').attr('value', comId);
					}
				}
			...
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_select_community]', null, error);
		}			
		return false;
	});
	...
	$('.js_remove_community').live('click', function(e) {
		try{
			...
			if(!isEmpty(searchFilter)){
				...
				departmentField.find('input[name="txtFilterStringOperand"]').attr('value', '');
			}
			if(!isEmpty(input.parents('td[fieldId="txtFromCommunity"]'))){
				var workTransfer = input.parents('.js_work_transfer_page');
			...
			smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search js_remove_community]', null, error);
		}			
		return false;
	});
});
}catch(error){
	smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-act-search script]', null, error);
}